...
class RemoteEntriesRepository implements Contract, ClearableRepository, PrunableRepository, TerminableRepository
{
    /**
     ...
     * @return void
     */
    public function __construct(string $connection, int $chunkSize = null, $remoteUrl = null)
    {
        ...
        if($this->remoteUrl == "")
        {
            throw new \Exception("TELESCOPE: Remote URL missing, Telescope will not function without it.");
        }
    }
    ...
    public function store(Collection $entries)
    {
        ...
        $entries->chunk($this->chunkSize)->each(function ($chunked) {
            Http::post($this->remoteUrl, $chunked->map(function ($entry) {
    ...
    protected function storeExceptions(Collection $exceptions)
    {
        $exceptions->chunk($this->chunkSize)->each(function ($chunked) {
            Http::post($this->remoteUrl, $chunked->map(function ($exception) {
    ...
    protected function storeTags(Collection $results)
    {
        $results->chunk($this->chunkSize)->each(function ($chunked) {
            try {
                Http::post($this->remoteUrl, $chunked->flatMap(function ($tags, $uuid) {
